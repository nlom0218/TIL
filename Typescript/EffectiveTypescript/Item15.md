...
`Props` 타입이 있다. 여기에서 인덱스 시그니처는 `[property: string]: string`에 해당한다. 이러한 인덱스 시그니처는 댜음 세 가지 의미를 담고 있다.
...
하지만 아직 아쉬운 점이 있다. 아직 모든 키들이 필수값이라는 것이다. 조건을 보자면 모두 필수값이 아니다. 이를 해결하기 위해선 앞에 `Partial`를 붙이면 된다.
...
> `string`이라는 광범위한 범위를 유니온을 사용하여 범위를 좁히면 더 이상 인덱스 시그니처라고 부를 수 없는게 아닐까?
